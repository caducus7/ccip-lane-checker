...
var WINNER_DECLARED_SIG = keccak2562(toBytes3("WinnerDeclared(uint256,uint8,uint256)"));
var onWinnerDeclared = (runtime2, log) => {
  ...
  runtime2.log(`WinnerDeclared round=${roundId} lane=${laneId} finishTime=${finishTime}`);
  const evmClient = createEvmClient(runtime2.config);
  const distributeTx = writeLaneController(runtime2, evmClient, laneControllerAbi, "distributePrizes", [roundId]);
  const result = {
  ...
  runtime2.log(`Prizes distributed: ${JSON.stringify(result)}`);
  return JSON.stringify(result);
};
var initWorkflow = (config) => {
  const selector = cre.capabilities.EVMClient.SUPPORTED_CHAIN_SELECTORS[config.chainSelectorName];
  if (selector === undefined) {
  ...
  return [
    handler(evmClient.logTrigger(logTriggerConfig({
      addresses: [config.laneControllerAddress],
