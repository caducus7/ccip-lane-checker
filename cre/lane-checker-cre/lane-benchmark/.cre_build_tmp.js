...
var laneLatencySchema = exports_external.object({
  lane: exports_external.object({
    sourceNetworkInfo: exports_external.object({
      displayName: exports_external.string().optional()
    }).optional(),
    destNetworkInfo: exports_external.object({
      displayName: exports_external.string().optional()
    }).optional()
  }).optional(),
  totalMs: exports_external.number().nonnegative()
});
var latencyAggregation = ConsensusAggregationByFields({
  totalMs: () => median()
});
var fetchLaneLatencyUrl = (sendRequester, url) => {
...
var buildLatencyUrl = (baseUrl, sourceChainSelector, destChainSelector) => `${baseUrl}/lanes/latency` + `?sourceChainSelector=${sourceChainSelector}` + `&destChainSelector=${destChainSelector}`;
var collectBenchmarks = (runtime2) => {
  const httpClient = new cre.capabilities.HTTPClient;
  const fetchedAt = runtime2.now().toISOString();
  const lanes = [];
  for (const route of runtime2.config.lanes) {
    const url = buildLatencyUrl(runtime2.config.ccipApiBaseUrl, route.sourceChainSelector, route.destChainSelector);
    try {
      const result = httpClient.sendRequest(runtime2, fetchLaneLatencyUrl, latencyAggregation)(url).result();
      lanes.push({
        label: route.label,
      ...
      runtime2.log(`lane ${route.label}: p90=${result.totalMs}ms (${route.sourceChainSelector}->${route.destChainSelector})`);
    } catch (error2) {
      ...
      runtime2.log(`lane ${route.label} failed: ${message}`);
    }
  }
  return {
    cacheKey: runtime2.config.cacheKey,
    fetchedAt,
    lanes
  };
};
var onCronTrigger = (runtime2, payload) => {
  ...
  runtime2.log(`lane-benchmark cron at ${scheduledAt}`);
  const snapshot = collectBenchmarks(runtime2);
  const output = JSON.stringify(snapshot);
  runtime2.log(`benchmark cache[${runtime2.config.cacheKey}]: ${output}`);
  return output;
};
var onHttpTrigger = (runtime2, triggerEvent) => {
  const bodyText = new TextDecoder().decode(triggerEvent.input);
  runtime2.log(`lane-benchmark HTTP refresh: ${bodyText}`);
  const snapshot = collectBenchmarks(runtime2);
  ...
  runtime2.log(`benchmark HTTP response: ${output}`);
  return output;
};
var initWorkflow = (config) => {
  ...
  return [
    ...
    handler(http.trigger({ authorizedKeys: config.authorizedKeys }), onHttpTrigger)
  ];
};
async function main() {
